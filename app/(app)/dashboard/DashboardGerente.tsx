...
import {
  ...
  DollarSign, RefreshCw, Clock, Send, Users,
} from 'lucide-react'
import type { VendedoraRankingMeta, DinheiroMesaInfo, ProdutoTopMes, ListaEsperaInfo } from './page'
...
export function DashboardGerente({
  ...
  const faltante = Math.max(META_MENSAL - vendidoMes, 0)
  const diasRestantesSafe = Math.max(diasRestantes, 1)
  const metaDiaria = faltante / diasRestantesSafe
  const pct = Math.min(Math.round((vendidoMes / META_MENSAL) * 100), 100)
  const metaBatida = vendidoMes >= META_MENSAL
    ...
    <div className="space-y-5 pb-10">
            ...
            <div className="flex-1 flex flex-col gap-5">
              <div>
                ...
                <p className="text-sm text-emerald-300/70 mt-2.5 font-medium">
                  potencial da operação em aberto
                </p>
                {totalPotencial === 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border bg-card shadow-sm p-5 flex flex-col gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            de {fmtVal(META_MENSAL)} da meta mensal
          </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border bg-card shadow-sm p-4">
          ...
          <p className="text-xl font-bold tabular-nums tracking-tight">{fmt(vendidoMes)}</p>
        ...
        <div className={`rounded-xl border shadow-sm p-4 ${
          ...
          <p className={`text-xl font-bold tabular-nums tracking-tight ${
            qtdAvisosAtrasados > 0 ? 'text-red-600 dark:text-red-400' : ''
          }`}>
            {qtdAvisosAtrasados}
          </p>
        ...
        <div className={`rounded-xl border shadow-sm p-4 ${
          ...
          <p className={`text-xl font-bold tabular-nums tracking-tight ${
            qtdAvisosHoje > 0 ? 'text-blue-600 dark:text-blue-400' : ''
          }`}>
            {qtdAvisosHoje}
          </p>
            ...
            <div className="space-y-1">
              {rankingMes.slice(0, 5).map((v, i) => {
                ...
                const status = pctV !== null
                  ...
                  : null
                const statusBadgeCls = pctV !== null
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        {status && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-none whitespace-nowrap ${statusBadgeCls}`}>
                            {status}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                          <div
                            className={`h-full rounded-full bg-gradient-to-r transition-all duration-700 ${barGradR}`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Link
          ...
          className="rounded-xl border bg-card shadow-sm p-4 flex items-center gap-3 hover:bg-muted/40 transition-colors"
        >
          <div className="w-9 h-9 rounded-lg bg-violet-50 dark:bg-violet-900/25 border border-violet-100 dark:border-violet-800/30 flex items-center justify-center flex-none">
