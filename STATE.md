...
- **SEC-0002B (Etapa B concluída, correção funcional pendente):** auditoria do SEC-0002 confirmou, e a Etapa B comprovou empiricamente em staging (`ynrffhacpjzohrhkpuiq`, migration `069_sec_0002b_etapa_b_prova_default_privileges.sql`, PR #5, merge commit `96fb9c3`), que `postgres` e `supabase_admin` têm `ALTER DEFAULT PRIVILEGES` configurado no schema `public` concedendo automaticamente privilégio total a `anon`/`authenticated`/`service_role` em qualquer tabela nova criada nesse schema. A migration de prova (efeito líquido zero no schema) está mesclada em `main`; não aplicada em produção. A correção funcional (`ALTER DEFAULT PRIVILEGES ... REVOKE`) ainda não foi iniciada — depende de gate próprio. Detalhes em HANDOFF.md.
